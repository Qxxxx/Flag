...
ax = plt.subplot(211, xlim=[0,53], ylim=[0,7], frameon=False, aspect=1)
...
for date in dates:
    ...
    valid = datetime(2021, 1, 1).weekday()
    X[day_of_year+valid-1] += 0.3 
calmap(ax, 2021, X.reshape(53,7).T)


plt.tight_layout()
plt.savefig("tmp/calendar-heatmap.png", dpi=300)
#plt.savefig("tmp/calendar-heatmap.pdf", dpi=600)
#plt.show()